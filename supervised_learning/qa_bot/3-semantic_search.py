...
""" implements a semantic search function using a pre-trained Universal
    Sentence Encoder. It allows users to input a query and find the most semantically
    similar document from a corpus of reference documents.
    ...
    Modules:
        - semantic_search(corpus_path, sentence): Identifies the most semantically 
          similar document from a corpus to a given query sentence.

    Usage:
        1. Prepare a directory containing text files (the corpus) that the system 
       will use as reference documents.
        2. Call the `semantic_search` function, providing the path to the corpus 
       directory and the query sentence.
        3. The function will return the most semantically similar document from the corpus.
"""
...
def semantic_search(corpus_path, sentence):
    """ finds the most semantically similar document to the given query using the Universal Sentence Encoder.
        ...
        Process:
            ...
            4. Compute the similarity score between the query and each document.
