...
data_generator = K.preprocessing.image.ImageDataGenerator(rotation_range=20,
...
cbs = []
save = K.callbacks.ModelCheckpoint('cifar10.h5', save_best_only=True, mode='max', monitor='val_accuracy')
cbs.append(save)
e_s = K.callbacks.EarlyStopping(patience=7)
cbs.append(e_s)
...
model.fit(x=data_generator.flow(px_train, y=py_train), epochs=50, batch_size=64, callbacks=cbs, shuffle=True, validation_data=p_test)