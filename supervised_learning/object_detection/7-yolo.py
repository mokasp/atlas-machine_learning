...
class Yolo():
    """ class that represents the YOLOv3 algorithm to perform object detection
            ...
            Public Instance Methods:
                ...
                predict(folder_path):
                    predicts bounding boxes, classes, and scores for
                    images in a folder
    ...
    def predict(self, folder_path):
        """
        predicts bounding boxes, classes, and scores for images in a folder

        PARAMETERS
        ==========
            folder_path - [str]:
                The path to the folder holding all the images to predict.

        RETURNS
        =======
            (predictions, image_paths) - [tuple]:
                    predictions - [list]:
                        A list of tuples for each image of
                        (boxes, box_classes, box_scores).
                    image_paths - [list]:
                        A list of image paths corresponding to each prediction
                        in predictions.
        """
        loaded_images, image_paths = Yolo.load_images(folder_path)
        images = {}
        processed_images, image_shapes = self.preprocess_images(loaded_images)
        for i in range(len(processed_images)):
            image_info = {}
            image_info['pimage'] = processed_images[i]
            image_info['og_shape'] = image_shapes[i]
            image_info['image_path'] = image_paths[i]

            path = os.path.split(image_paths[i])
            images[path[len(path) - 1]] = image_info
        predictions = []
        i = 0
        for image in images:
            og = images[image]['pimage']
            new = og.reshape((1, 416, 416, 3))
            prediction = self.model.predict(new)
            p = []
            for x in range(3):
                p.append(prediction[x][0])

            size = images[image]['og_shape']

            boxes, box_confidences, box_class_probs = \
                self.process_outputs(p, size)
            boxes, box_confidences, box_class_probs = self.filter_boxes(
                boxes, box_confidences, box_class_probs)
            boxes, box_classes, box_scores = self.non_max_suppression(
                boxes, box_confidences, box_class_probs)
            predictions.append((boxes, box_classes, box_scores))
            self.show_boxes(loaded_images[i],
                            boxes,
                            box_classes,
                            box_scores,
                            image)
