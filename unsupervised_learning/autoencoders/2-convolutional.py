...
def autoencoder(input_dims, filters, latent_dims):
    ...
    for i in range(layers):
        layer = K.layers.Conv2D(filters[i], (3, 3), padding='same', activation='relu')(layer)
    ...
    for i in range(layers):
        if i != layers - 1:
            layer = K.layers.Conv2D(decode_layers[i], (3, 3), padding='same', activation='relu')(layer)
            layer = K.layers.UpSampling2D((2, 2))(layer)
        else:
            layer = K.layers.Conv2D(decode_layers[i], (3, 3), padding='valid', activation='relu')(layer)
            layer = K.layers.UpSampling2D((2, 2))(layer)
    latent = K.layers.Conv2D(input_dims[2], (3, 3), activation='sigmoid', padding='same')(layer)
