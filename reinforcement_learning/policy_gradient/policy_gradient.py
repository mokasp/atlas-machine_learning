...
def softmax_grad(softmax):
    ...
    # calculate the softmax gradient by subtracting the outer product from the diagonal matrix
...
def policy_gradient(state, weight):
    ...
    # get gradient of softmax probabilities at the choosen action, which indicates how the action probabilities change
    softmax_g = softmax_grad(action_probs)[action, :]
    
    # caclulate the log probabilities of the choosen actions based on the softmax gradient
    # normalizing the gradient of softmax function relative to the pobability of the choosen action
    log_probs = softmax_g / action_probs[0, action]
   
    # get the gradient of the policy which will be used to update the policies weights
    # add extra dimension to the log probabilities
